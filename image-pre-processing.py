...
for dirpath, dirnames, filenames in walk(path.join(path.curdir, src)):
    for index, filename in tqdm(
        zip(range(dest_length, dest_length + len(filenames)), filenames),
        total=len(filenames),
        bar_format=" {l_bar}{bar}| {n_fmt}/{total_fmt} [{elapsed}] ",
    ):
        start = time()

        img = io.imread(path.join(dirpath, filename))
